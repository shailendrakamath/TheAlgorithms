...
public final class ClosestPair {
	...
	private static final java.util.logging.Logger LOGGER = java.util.logging.Logger.getLogger(ClosestPair.class.getName());
	...
	public double closestPair(final Location[] a, final int indexNum) {
		...
		Location[] firstWindow = new Location[totalNum];
		for (int i = 0; i < totalNum; i++) {
			double xGap = Math.abs(divideArray[divideX].x - divideArray[i].x);
			if (xGap >= minValue && divideArray[i].x > divideArray[divideX].x) {
				break;
			}
	...
	public double bruteForce(final Location[] arrayParam) {
		...
		if (arrayParam.length == 3) {
			for (int i = 0; i < arrayParam.length - 1; i++) {
				// Difference between x coordinates
				xGap = (arrayParam[i].x - arrayParam[i + 1].x);
				// Difference between y coordinates
				yGap = (arrayParam[i].y - arrayParam[i + 1].y);
				// distance between coordinates
				length = Math.sqrt(Math.pow(xGap, 2) + Math.pow(yGap, 2));
				// If measured distance is less than current min distance
				if (length < minValue) {
	...
	public static void main(final String[] args) {
		...
		LOGGER.log(Level.INFO, "Number of points: {0}", cp.array.length);
		for (int i=0;i<cp.array.length;i++){
			LOGGER.log(Level.INFO, "x: {0}, y: {1}", new Object[]{cp.array[i].x, cp.array[i].y});
		}
		...
		LOGGER.log(Level.INFO, "({0}, {1})", new Object[]{cp.point1.x, cp.point1.y});
		LOGGER.log(Level.INFO, "({0}, {1})", new Object[]{cp.point2.x, cp.point2.y});
		LOGGER.log(Level.INFO, "Minimum Distance : {0}", result);
